...
const FlapCell = React.memo(
  function FlapCell({
    ...
    return (
      <div className="flex flex-col overflow-hidden rounded-sm border border-neutral-300 dark:border-black" style={{ aspectRatio: "3/6" }}>
        <div className="relative flex-1" style={{ perspective: "1000px", transformStyle: "preserve-3d" }}>
          ...
          <div
            className={cn(
              "absolute inset-x-0 top-0 overflow-hidden rounded-t-sm",
              topBg
            )}
            style={{ height: "calc(50% - 0.5px)" }}
          >
            <div
              className={cn(textCx, textColor, "top-0")}
              style={{ ...CELL_TEXT_STYLE, position: "absolute", height: "200%" }}
            >
              {show}
            </div>
          </div>
          ...
          <div
            className={cn(
              "absolute inset-x-0 bottom-0 overflow-hidden rounded-b-sm",
              bottomBg
            )}
            style={{ height: "calc(50% - 0.5px)" }}
          >
            <div
              className={cn(textCx, textColor, "bottom-0")}
              style={{ ...CELL_TEXT_STYLE, position: "absolute", height: "200%" }}
            >
              {show}
            </div>
          ...
          {flipId > 0 && (
            <motion.div
              key={flipId}
              className={cn(
                "absolute inset-x-0 top-0 z-10 overflow-hidden rounded-t-sm",
                flapTopBg
              )}
              style={{
            ...
            >
              <div
                className={cn(textCx, flapTextColor, "top-0")}
                style={{
                  ...CELL_TEXT_STYLE,
                  position: "absolute",
                  height: "200%",
          ...
          {flipId > 0 && (
            <motion.div
              key={`b${flipId}`}
              className={cn(
                "absolute inset-x-0 bottom-0 z-10 overflow-hidden rounded-b-sm",
                bottomBg
              )}
              style={{
                height: "calc(50% - 0.5px)",
          ...
          <div className="pointer-events-none absolute inset-x-0 top-1/2 z-20 h-px bg-neutral-400/50 dark:bg-black/50" style={{ transform: "translateY(-0.5px)" }} />
        </div>

        <div
          className="w-full text-neutral-400 opacity-20 dark:text-black dark:opacity-100"
          style={{
            height: "1rem",
            backgroundImage:
              "repeating-linear-gradient(to bottom, currentColor 0, currentColor 1px, transparent 1px, transparent 0.2rem)",
          }}
        />
      </div>
    );
...
const ColorCell = React.memo(function ColorCell({ color }: { color: string }) {
  return (
    <div
      className="rounded-sm border-2 border-neutral-300 dark:border-black"
